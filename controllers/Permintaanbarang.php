...
class Permintaanbarang extends CI_Controller
{
	...
	public function index()
	{
		$data = array(
			...
			'permintaankabag' => $this->mp->getDatapermintaankabag(),
		);
	...
	public function setujuikabagPermintaan($id)
	{
		$id = $this->uri->segment(3);
		$cek = $this->mp->getPermintaan($id);

		foreach ($cek as $row) {
			$c = $row['status_kasubag'];
		}
		// $status_kasubag = $cek['status_kasubag'];
		$status_kasubag = $c;
		if ($status_kasubag == 1) {
			...
			$this->mp->storepengeluaran($pengeluaran);
			$data['status_kabag'] = '1';
			unset($data['id']);
			$result = $this->mp->updatePermintaan($id, $data);
			if ($result >= 1) {
			...
			$this->session->set_flashdata('gagal', 'Harus Disetujui Dulu Oleh Kasubag');
			redirect($_SERVER['HTTP_REFERER'], 'refresh');
		}
	}
	...
	public function tolakkabagPermintaan($id)
	{
		$tgl_permintaan = $this->input->post('tgl_permintaan');
		$ruang = $this->input->post('ruang');
		$id = $this->uri->segment(3);
