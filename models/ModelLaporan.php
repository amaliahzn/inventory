...
class ModelLaporan extends CI_Model
{
    ...
    public function getbarangmasuk($tanggal_awal, $tanggal_akhir)
    {
        $this->db->select('*');
        $this->db->from('barangmasuk_detail');
        // $this->db->join('pelanggan b', 'b.id_pelanggan = a.id_pelanggan');
        // $this->db->join('paket c', 'c.id_paket = a.id_paket');
        $this->db->where('tanggal >=', $tanggal_awal);
        $this->db->where('tanggal <=', $tanggal_akhir);
        $query = $this->db->get();
        return $query->result_array();
    }
    public function getbarangkeluar($tanggal_awal, $tanggal_akhir)
    {
        $this->db->select('*');
    ...
    public function getstokbarang($tanggal_awal, $tanggal_akhir)
    {
        $this->db->select('*');
        $this->db->from('barangmasuk_detail');
        $this->db->join('barangkeluar_detail b', 'b.kd_barang = a.kd_barang');
        // $this->db->join('paket c', 'c.id_paket = a.id_paket');
        $this->db->where('tanggal >=', $tanggal_awal);
        $this->db->where('tanggal <=', $tanggal_akhir);
        $query = $this->db->get();
